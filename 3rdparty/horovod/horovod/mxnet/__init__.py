...
class DistributedOptimizer(mx.optimizer.Optimizer):
    def __init__(self, optimizer, num_groups=None, skip_average=False):
        self._optimizer = optimizer
        self._num_groups = num_groups
        self._skip_average = skip_average
        ...
        if isinstance(index, (tuple, list)):
            if (self._num_groups and self._num_groups > 0):
                ...
                for i in range(len(index)):
                    allreduce_(grad[i], average=(not self._skip_average), name=str(index[i]),
                               group_id=group_ids[i // num_grads_per_group])
            else:
                for i in range(len(index)):
                    allreduce_(grad[i], average=(not self._skip_average), name=str(index[i]))
        else:
            allreduce_(grad, average=(not self._skip_average), name=str(index))
