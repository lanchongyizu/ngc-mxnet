...
void DoAllreduce(NDArray* tensor, NDArray* output, const std::string& name,
                 Callback on_complete, const int group_id) {
...
extern "C" int horovod_mxnet_allreduce_async(NDArray* input, NDArray* output,
  ...
  auto allreduce_async_fn = [input, output,
                             op_name, group_id](RunContext rctx,
                                      Callback on_complete) mutable {
    DoAllreduce(input, output, op_name, on_complete, group_id);
  };

#if HAVE_CUDA
  auto allreduce_async_cpu_fn =
      [input, output, op_name, group_id](RunContext rctx,
                               Callback on_complete) mutable {
        DoAllreduceCudaOnCPU(input, output, op_name, on_complete, group_id);
      };
#endif
  ...
  // Not in-place
  if (input->var() != output->var()) {
    Engine::Get()->PushAsync(allreduce_async_fn, Context::CPU(), {input->var()},
                             {output->var()}, FnProperty::kNormal, 0,
                             "HorovodAllreduce");
  // In-place
  } else {
...
extern "C" int horovod_mxnet_broadcast_async(NDArray* input, NDArray* output,
  ...
  // Not in-place
  if (input->var() != output->var()) {
    Engine::Get()->PushAsync(broadcast_async_fn, Context::CPU(), {input->var()},
                             {output->var()}, FnProperty::kNormal, 0,
                             "HorovodBroadcast");
  // In-place
  } else {
