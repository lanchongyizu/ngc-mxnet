...
# Previous- should nuke soon
#ifeq ($(USE_TENSORRT), 1)
#	CFLAGS +=  -I$(ROOTDIR) -I/usr/include -L/usr/local/lib -I$(TPARTYDIR) -I$(ROOTDIR)/onnx-tensorrt/third_party/onnx -lprotobuf -pthread -lonnx -lonnx_proto -lnvonnxparser -lnvonnxparser_runtime -lnvinfer -lnvinfer_plugin -DONNX_NAMESPACE=$(ONNX_NAMESPACE) -DMXNET_USE_TENSORRT=1
#	LDFLAGS += -I/usr/include -L/usr/local/lib -lprotobuf -pthread -lonnx -lonnx_proto -lnvonnxparser -lnvonnxparser_runtime -lnvinfer -lnvinfer_plugin
#endif 
...
ifeq ($(USE_HOROVOD),1)
	HVDOBJ = $(addprefix $(HOROVOD_PATH)/build/common/, common.o mpi_message.o operations.o timeline.o batched_memcpy.o)
...
# Temporary verbose compile of this file to check occupancy (num_registers should be <= 64)
build/src/operator/nn/batch_norm_gpu.o: src/operator/nn/batch_norm.cu
	@mkdir -p $(@D)
	$(NVCC) $(NVCCFLAGS) --ptxas-options=-v $(CUDA_ARCH) -Xcompiler "$(CFLAGS)" -M -MT $*.o $< >$*.d
	$(NVCC) -c -o $@ $(NVCCFLAGS) --ptxas-options=-v $(CUDA_ARCH) -Xcompiler "$(CFLAGS)" $<

# Temporary verbose compile of this file to check occupancy (num_registers should be <= 64)
build/src/operator/nn/batch_norm_add_relu_gpu.o: src/operator/nn/batch_norm_add_relu.cu
	@mkdir -p $(@D)
	$(NVCC) $(NVCCFLAGS) --ptxas-options=-v $(CUDA_ARCH) -Xcompiler "$(CFLAGS)" -M -MT $*.o $< >$*.d
	$(NVCC) -c -o $@ $(NVCCFLAGS) --ptxas-options=-v $(CUDA_ARCH) -Xcompiler "$(CFLAGS)" $<
...
$(HOROVOD_PATH)/build/common/%.o: $(HOROVOD_PATH)/horovod/common/%.cu
	@mkdir -p $(@D)
	$(NVCC) -c -o $@ $(NVCCFLAGS) $(CUDA_ARCH) -Xcompiler "$(CFLAGS)" $<
