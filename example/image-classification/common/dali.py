...
def add_dali_args(parser):
    group = parser.add_argument_group('DALI', 'pipeline and augmentation')
    group.add_argument('--use-dali', action='store_true',
                       help='use dali pipeline and augmentation')
    group.add_argument('--separ-val', action='store_true',
                       help='each process will perform independent validation on whole val-set')
    group.add_argument('--dali-threads', type=int, default=3,
                       help='number of threads per GPU for DALI')
    group.add_argument('--dali-prefetch-queue', type=int, default=3,
                       help='DALI prefetch queue depth')
    group.add_argument('--dali-nvjpeg-memory-padding', type=int, default=16,
                       help='Memory padding value for nvJPEG (in MB)')
    group.add_argument('--resize', type=int, default=256,
                       help='Shortest image edge size after resizing')
    return parser
...
def get_rec_iter(args, kv=None):
    ...
    dali_val_iter = DALIClassificationIterator(valpipes, valpipes[0].epoch_size("Reader") // (1 if args.separ_val else num_workers), fill_last_batch = False) if args.data_val else None
