...
def add_dali_args(parser):
    ...
    group.add_argument('--synthetic', type=int, default=0,
...
def get_rec_iter(args, kv=None):
    ...
    if 'horovod' in args.kvstore:
        gpus = [hvd.local_rank()]
    else:
        gpus = range(args.n_GPUs)
    ...
    if 'synthetic' in args and args.synthetic == 1:
        ...
        trainpipes = [HybridTrainPipe(batch_size     = batch_size,
                                      ...
                                      device_id      = gpu_id,
                                      rec_path       = args.data_dir + "/train.rec",
                                      idx_path       = args.data_dir + "/train.idx",
        ...
        valpipes = [HybridValPipe(batch_size     = batch_size,
                                  num_threads    = num_threads,
                                  device_id      = gpu_id,
                                  rec_path       = args.data_dir + "/val.rec",
                                  idx_path       = args.data_dir + "/val.idx",
                                  ...
                                  dtype          = args.dtype,
                                  nvjpeg_padding = args.dali_nvjpeg_memory_padding * 1024 * 1024,
                                  prefetch_queue = args.dali_prefetch_queue) for gpu_id in gpus] if not args.no_val else None
        trainpipes[0].build()
        if not args.no_val:
            valpipes[0].build()
        ...
        dali_val_iter = DALIClassificationIterator(valpipes, valpipes[0].epoch_size("Reader") // (1 if args.separ_val else num_workers), fill_last_batch = False) if not args.no_val else None
