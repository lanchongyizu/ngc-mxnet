...
struct ApproxGradientParam : public dmlc::Parameter<ApproxGradientParam> {
  ...
  DMLC_DECLARE_PARAMETER(ApproxGradientParam) {
    DMLC_DECLARE_FIELD(index)
      .set_lower_bound(0)
      .describe("Index of gradient vector coordinate to be calculated");
...
void ApproxGradient(const nnvm::NodeAttrs& attrs,
  ...
  const size_t workspace_total_bytes = (num_items + 1) * sizeof(float) + extraStorageBytes;
