...
class CuDNNNormalizedConvolutionOp {
 public:
  CuDNNNormalizedConvolutionOp() {
    ...
    // New fused forward-op descriptor creation
    CUDNN_CALL(cudnnCreateFusedOpsPlan(&fwd_op_,
      CUDNN_FUSED_SCALE_BIAS_ACTIVATION_CONV_BNSTATS));
    CUDNN_CALL(cudnnCreateFusedOpsConstParamPack(&fwd_op_params_,
      CUDNN_FUSED_SCALE_BIAS_ACTIVATION_CONV_BNSTATS));
    CUDNN_CALL(cudnnCreateFusedOpsVariantParamPack(&fwd_op_data_ptrs_,
      CUDNN_FUSED_SCALE_BIAS_ACTIVATION_CONV_BNSTATS));
    CUDNN_CALL(cudnnCreateActivationDescriptor(&activation_desc_));

    // New fused backward-wgrad-op descriptor creation
    CUDNN_CALL(cudnnCreateFusedOpsPlan(&bwd_wgrad_op_,
      CUDNN_FUSED_SCALE_BIAS_ACTIVATION_WGRAD));
    CUDNN_CALL(cudnnCreateFusedOpsConstParamPack(&bwd_wgrad_op_params_,
      CUDNN_FUSED_SCALE_BIAS_ACTIVATION_WGRAD));
    CUDNN_CALL(cudnnCreateFusedOpsVariantParamPack(&bwd_wgrad_op_data_ptrs_,
      CUDNN_FUSED_SCALE_BIAS_ACTIVATION_WGRAD));
#endif
  }
...
#if CUDNN_VERSION >= 7600
    // New fused forward-op descriptor destruction
    CUDNN_CALL(cudnnDestroyFusedOpsVariantParamPack(fwd_op_data_ptrs_));
    CUDNN_CALL(cudnnDestroyFusedOpsConstParamPack(fwd_op_params_));
    CUDNN_CALL(cudnnDestroyFusedOpsPlan(fwd_op_));
    CUDNN_CALL(cudnnDestroyActivationDescriptor(activation_desc_));

    // New fused backward-wgrad-op descriptor destruction
    CUDNN_CALL(cudnnDestroyFusedOpsVariantParamPack(bwd_wgrad_op_data_ptrs_));
    CUDNN_CALL(cudnnDestroyFusedOpsConstParamPack(bwd_wgrad_op_params_));
    CUDNN_CALL(cudnnDestroyFusedOpsPlan(bwd_wgrad_op_));
#endif
  ...
  void Forward(const OpContext &ctx,
    ...
    cudnnSetFusedOpsVariantParamPackAttribute(fwd_op_data_ptrs_,
                                              CUDNN_PTR_XDATA, data_ptr);
    cudnnSetFusedOpsVariantParamPackAttribute(fwd_op_data_ptrs_,
                                              CUDNN_PTR_WDATA, wmat_ptr);
    cudnnSetFusedOpsVariantParamPackAttribute(fwd_op_data_ptrs_,
                                              CUDNN_PTR_BN_EQSCALE, equiv_scale_ptr);
    cudnnSetFusedOpsVariantParamPackAttribute(fwd_op_data_ptrs_,
                                              CUDNN_PTR_BN_EQBIAS, equiv_bias_ptr);

    // Set workspace input pointer in op instance
    cudnnSetFusedOpsVariantParamPackAttribute(fwd_op_data_ptrs_,
                                              CUDNN_PTR_WORKSPACE, workspace.dptr_);
    cudnnSetFusedOpsVariantParamPackAttribute(fwd_op_data_ptrs_,
    ...
    cudnnSetFusedOpsVariantParamPackAttribute(fwd_op_data_ptrs_,
                                              CUDNN_PTR_YDATA, out_ptr);
    cudnnSetFusedOpsVariantParamPackAttribute(fwd_op_data_ptrs_,
                                              CUDNN_PTR_YSUM, sum_ptr);
    cudnnSetFusedOpsVariantParamPackAttribute(fwd_op_data_ptrs_,
                                              CUDNN_PTR_YSQSUM, sum_of_squares_ptr);

    // Launch forward operation
    CUDNN_CALL(cudnnFusedOpsExecute(s->dnn_handle_, fwd_op_, fwd_op_data_ptrs_));
#endif  // CUDNN_VERSION < 7600
  }
  ...
  void Backward(const OpContext &ctx,
    ...
    // This operator does not support output blending as specified by alpha or beta.
    // Set data input pointers in op instance
    cudnnSetFusedOpsVariantParamPackAttribute(bwd_wgrad_op_data_ptrs_,
                                              CUDNN_PTR_XDATA, data_ptr);
    cudnnSetFusedOpsVariantParamPackAttribute(bwd_wgrad_op_data_ptrs_,
                                              CUDNN_PTR_DYDATA, y_grad_ptr);
    if (!param_.no_equiv_scale_bias || supply_constants_) {
    ...
    // Set data output pointers in op instance
    cudnnSetFusedOpsVariantParamPackAttribute(bwd_wgrad_op_data_ptrs_,
                                         CUDNN_PTR_DWDATA, wgt_grad_ptr);

    // Launch backward wgrad operation into the alternate stream (if enabled)
    CUDNN_CALL(cudnnFusedOpsExecute(s_wgrad.GetStream()->dnn_handle_,
                                    bwd_wgrad_op_, bwd_wgrad_op_data_ptrs_));
 ...
 */
  ...
  void InitDescriptors(const std::vector<TShape>& in_shape,
    ...
    // Describe input tensor pointers and alignment for forward fused op
    SetOpParamVal(fwd_op_params_, CUDNN_PARAM_XDATA_PLACEHOLDER, CUDNN_PTR_16B_ALIGNED);
    SetOpParamVal(fwd_op_params_, CUDNN_PARAM_WDATA_PLACEHOLDER, CUDNN_PTR_16B_ALIGNED);
    SetOpParamVal(fwd_op_params_, CUDNN_PARAM_BN_EQSCALE_PLACEHOLDER, eq_scale_bias_ptr_type);
    SetOpParamVal(fwd_op_params_, CUDNN_PARAM_BN_EQBIAS_PLACEHOLDER, eq_scale_bias_ptr_type);
    // Describe output tensor pointers and their alignments for forward fused op
    SetOpParamVal(fwd_op_params_, CUDNN_PARAM_YDATA_PLACEHOLDER, CUDNN_PTR_16B_ALIGNED);
    auto stats_ptr_type = output_stats ? CUDNN_PTR_16B_ALIGNED : CUDNN_PTR_NULL;
    SetOpParamVal(fwd_op_params_, CUDNN_PARAM_YSUM_PLACEHOLDER, stats_ptr_type);
    SetOpParamVal(fwd_op_params_, CUDNN_PARAM_YSQSUM_PLACEHOLDER, stats_ptr_type);

    // Describe input tensor pointers and alignment for backward wgrad fused op
    SetOpParamVal(bwd_wgrad_op_params_, CUDNN_PARAM_DYDATA_PLACEHOLDER, CUDNN_PTR_16B_ALIGNED);
    SetOpParamVal(bwd_wgrad_op_params_, CUDNN_PARAM_XDATA_PLACEHOLDER, CUDNN_PTR_16B_ALIGNED);
    SetOpParamVal(bwd_wgrad_op_params_, CUDNN_PARAM_BN_EQSCALE_PLACEHOLDER, eq_scale_bias_ptr_type);
    SetOpParamVal(bwd_wgrad_op_params_, CUDNN_PARAM_BN_EQBIAS_PLACEHOLDER, eq_scale_bias_ptr_type);
    // Describe output tensor pointers and their alignments for backward wgrad fused op
    SetOpParamVal(bwd_wgrad_op_params_, CUDNN_PARAM_DWDATA_PLACEHOLDER, CUDNN_PTR_16B_ALIGNED);
    ...
    CUDNN_CALL(cudnnSetConvolutionMathType(fwd_conv_desc_, math_type));
    CUDNN_CALL(cudnnSetConvolutionGroupCount(fwd_conv_desc_, param_.num_group));
    ...
    if (!param_.no_equiv_scale_bias || supply_constants_) {
      SetOpParamDesc(fwd_op_params_,
                     CUDNN_PARAM_BN_EQSCALEBIAS_DESC, equiv_scale_bias_desc_);
      SetOpParamDesc(bwd_wgrad_op_params_,
                     CUDNN_PARAM_BN_EQSCALEBIAS_DESC, equiv_scale_bias_desc_);
    }
    ...
    if (output_stats) {
      ...
      CUDNN_CALL(cudnnSetTensorNdDescriptor(out_stats_desc_,
                                          ...
                                          &stats_shape[0],
                                          &stats_stride[0]));
      SetOpParamDesc(fwd_op_params_, CUDNN_PARAM_YSTATS_DESC, out_stats_desc_);
    }
    ...
    CUDNN_CALL(cudnnSetConvolutionMathType(fwd_conv_desc_, CUDNN_TENSOR_OP_MATH));
    ...
    if (mode != CUDNN_ACTIVATION_IDENTITY) {
      SetOpParamDesc(fwd_op_params_,
                     CUDNN_PARAM_ACTIVATION_DESC, activation_desc_);
      SetOpParamDesc(bwd_wgrad_op_params_,
                     CUDNN_PARAM_ACTIVATION_DESC, activation_desc_);
    }
    ...
    SetOpParamDesc(fwd_op_params_,
                   CUDNN_PARAM_XDESC, in_desc_);
    SetOpParamDesc(bwd_wgrad_op_params_,
                   CUDNN_PARAM_XDESC, in_desc_);
    // FusedOp does not accept CUDNN_BATCHNORM_SPATIAL_PERSISTENT
    SetOpParamVal(fwd_op_params_,
                  CUDNN_PARAM_BN_MODE, CUDNN_BATCHNORM_SPATIAL);
    SetOpParamVal(bwd_wgrad_op_params_,
                  CUDNN_PARAM_BN_MODE, CUDNN_BATCHNORM_SPATIAL);

    // The NVIDIA-version of the Convolution op provides parameters for controlling math precision
    ...
    SetOpParamDesc(fwd_op_params_,
                   CUDNN_PARAM_CONV_DESC, fwd_conv_desc_);
    SetOpParamDesc(bwd_wgrad_op_params_,
                   CUDNN_PARAM_CONV_DESC, fwd_conv_desc_);
    // W desc for forward == dW desc for backward wgrad
    SetOpParamDesc(fwd_op_params_,
                   CUDNN_PARAM_WDESC, filter_desc_);
    SetOpParamDesc(bwd_wgrad_op_params_,
                   CUDNN_PARAM_DWDESC, filter_desc_);
    // Y desc for forward == dY desc for backward wgrad
    SetOpParamDesc(fwd_op_params_,
                   CUDNN_PARAM_YDESC, out_desc_);
    SetOpParamDesc(bwd_wgrad_op_params_,
                   CUDNN_PARAM_DYDESC, out_desc_);
#endif  // CUDNN_VERSION < 7600
  ...
  void GetTempSize(const OpContext& ctx) {
    ...
    CUDNN_CALL(cudnnMakeFusedOpsPlan(s->dnn_handle_, fwd_op_, fwd_op_params_,
                                     &fwd_workspace_byte_));
    // Make op plan for backward wgrad op and set backward wgrad workspace size
    CUDNN_CALL(cudnnMakeFusedOpsPlan(s->dnn_handle_, bwd_wgrad_op_, bwd_wgrad_op_params_,
...
#if CUDNN_VERSION >= 7600
  // New forward fused-op and descriptors
  cudnnFusedOpsPlan_t fwd_op_;
  // Op parameters are held in a 'parameter pack' of descriptor info and data ptr alignments.
  cudnnFusedOpsConstParamPack_t fwd_op_params_;
  // Op I/O data ptrs are held in a 'pointer pack'
  cudnnFusedOpsVariantParamPack_t fwd_op_data_ptrs_;
  // Specifies activation parameters: relu
  cudnnActivationDescriptor_t activation_desc_;

  // New fused backward-wgrad fused-op and descriptors
  cudnnFusedOpsPlan_t bwd_wgrad_op_;
  // Op parameters are held in a 'parameter pack' of descriptor info and data ptr alignments.
  cudnnFusedOpsConstParamPack_t bwd_wgrad_op_params_;
  // Op I/O data ptrs are held in a 'pointer pack'
  cudnnFusedOpsVariantParamPack_t bwd_wgrad_op_data_ptrs_;
#endif
};
