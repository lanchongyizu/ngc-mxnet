...
static bool PoolingShape(const nnvm::NodeAttrs &attrs,
  ...
  if (param.global_pool) {
    ...
    if (param.pooling_convention == pool_enum::kValid) {
      oshape_ncw[2] = 1 + (dshape_ncw[2] + 2 * param.pad[0] - param.kernel[0]) /
                          param.stride[0];
    } else {
      oshape_ncw[2] = 1 + static_cast<int>(ceil(static_cast<float>(
    ...
    if (param.pooling_convention == pool_enum::kValid) {
      ...
      oshape_nchw[2] = 1 + static_cast<int>(ceil(static_cast<float>(
                                                     dshape_nchw[2] + 2 * param.pad[0] -
                          param.kernel[0]) / param.stride[0]));
      oshape_nchw[3] = 1 + static_cast<int>(ceil(static_cast<float>(
                                                     dshape_nchw[3] + 2 * param.pad[1] -
    ...
    if (param.pooling_convention == pool_enum::kValid) {
      ...
      oshape_ncdhw[2] = 1 + static_cast<int>(ceil(static_cast<float>(
                          dshape_ncdhw[2] + 2 * param.pad[0] -
                          param.kernel[0]) / param.stride[0]));
      oshape_ncdhw[3] = 1 + static_cast<int>(ceil(static_cast<float>(
                          dshape_ncdhw[3] + 2 * param.pad[1] -
                          param.kernel[1]) / param.stride[1]));
      oshape_ncdhw[4] = 1 + static_cast<int>(ceil(static_cast<float>(
                          dshape_ncdhw[4] + 2 * param.pad[2] -
