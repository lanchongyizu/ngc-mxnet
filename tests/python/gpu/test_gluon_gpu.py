...
def _test_gluon_bulking_in_process(seed, time_per_iteration):
...
def _test_bulking(test_bulking_func):
    ...
    blksz = 100
    test_cases = [(0,0,True), (1,1,True), (blksz,blksz,False),
                  (blksz,2,True), (2,blksz,True),
                  (blksz,blksz,True)]
    times = {}
    times_str = ''
    for seg_sizes in test_cases:
        ...
        times_str += \
            '\n    runtime of op seg bulking (fwd,bwd,enable)=({},{},{}):\t{:.1f} msec'.format(
                seg_sizes[0], seg_sizes[1], seg_sizes[2], 1000.0 * times[seg_sizes])

    fastest_non_bulked_time = min(times[(0,0,True)], times[(1,1,True)], times[(blksz,blksz,False)])
    slowest_half_bulked_time = max(times[(2,blksz,True)], times[(blksz,2,True)])
    fastest_half_bulked_time = min(times[(2,blksz,True)], times[(blksz,2,True)])
    ...
    sys.stderr.flush()
    # Non-bulked times[0,0,True], times[1,1,True] and times[blksz,blksz,False] should be comparable,
    # and slower than both half-bulked times[2,blksz,True] and times[blksz,2,True]
...
@with_seed()
def test_bulking():
    _test_bulking(_test_gluon_bulking_in_process)
