...
def test_rsp_push_pull():
    ...
    def check_rsp_push_pull(kv_type, sparse_pull, is_push_cpu=True):
        ...
        num_cpu_ctxs = 4
        check_rsp_pull(kv, num_gpu_ctxs, [mx.gpu(i//2) for i in range(num_gpu_ctxs)], sparse_pull)
        check_rsp_pull(kv, num_gpu_ctxs, [mx.gpu(i//2) for i in range(num_gpu_ctxs)], sparse_pull, is_same_rowid=True)
        check_rsp_pull(kv, num_cpu_ctxs, [mx.cpu(i) for i in range(num_cpu_ctxs)], sparse_pull)
        check_rsp_pull(kv, num_cpu_ctxs, [mx.cpu(i) for i in range(num_cpu_ctxs)], sparse_pull, is_same_rowid=True)
        check_rsp_pull(kv, num_gpu_ctxs, [mx.gpu(i//2) for i in range(num_gpu_ctxs)], sparse_pull, use_slice=True)
        check_rsp_pull(kv, num_cpu_ctxs, [mx.cpu(i) for i in range(num_cpu_ctxs)], sparse_pull, use_slice=True)
